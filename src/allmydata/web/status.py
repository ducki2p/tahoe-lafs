...
def tfmt(when):
    #return when * 1000.0 # stupid JS timestamps
    return "%.6f" % when
    # the timeline markers represent UTC. To make these events line up, we
    # must supply the "Z" suffix.
    #return "%.2f" % (1000*when)
    t = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(when))
    fract = "%.6f" % (when % 1.0)
    if fract.startswith("0."):
        fract = fract[2:]
    return t+"."+fract+"Z"
...
class DownloadStatusPage(DownloadResultsRendererMixin, rend.Page):
    ...
    def child_timeline_parameters(self, ctx):
        ds = self.download_status
        d = { "start": tfmt(ds.started),
              "end": tfmt(ds.started+2.0),
              }
    ...
    def render_events(self, ctx, data):
        ...
        t = T.table(class_="status-download-events")
        t[T.tr[T.td["segnum"], T.td["start"], T.td["active"], T.td["finish"],
               T.td["range"],
        ...
        t[T.tr[T.td["serverid"], T.td["shnum"], T.td["range"],
               T.td["txtime"], T.td["rxtime"],
               T.td["received"], T.td["RTT"]]]
